...
describe("reapOrphans", () => {
	type Rec = { ownerPid: number; browserPid?: number };
	...
	it("leaves a live owner's profile completely alone", () => {
		const { killed, removed } = harness({
			entries: ["200-a"],
			owners: { "200-a": { ownerPid: 200, browserPid: 900 } },
			alive: [200],
			live: { "200-a": [900] },
		});
	...
	it("rediscovers the kill target when no browser pid was recorded", () => {
		const { killed, removed } = harness({
			entries: ["200-a"],
			owners: { "200-a": { ownerPid: 200 } }, // crashed before recording
			alive: [],
			live: { "200-a": [903] },
		});
	...
	it("never throws when a kill or a removal fails, and keeps sweeping", () => {
		...
		expect(() =>
			reapOrphans({
				...
				readOwner: (dir) =>
					dir.endsWith("200-a") ? { ownerPid: 1, browserPid: 9 } : undefined,
				isPidAlive: () => false,
