...
import { whyUnusableClocks } from "../../clock/index.js";
import {
	boundsFor,
	REVIEWER_ANSWER_MS,
...
// Held equal to the pi adapter's own subagent runner default
// (`DEFAULT_RUN_PI_TIMEOUT_MS`, 45 minutes) — see the rationale on
// `REVIEWER_BACKSTOP_MS` in budget.ts for why this is inlined here
// rather than imported from a runner this library has no opinion
// about.
const DEFAULT_RUN_PI_TIMEOUT_MS = 45 * 60 * 1000;
...
describe("retryWouldRepeat with a keptBy participant", () => {
	it("names the participant's own knob when it keeps its own clocks", () => {
		const said = retryWouldRepeat(
			{ limit: "wall-clock", detail: "x", budgetMs: 900_000 },
			{ timeoutMs: 900_000, idleTimeoutMs: 600_000 },
			"reviewer-a",
		);
		expect(said).toContain('"reviewer-a"');
	});
});
