 ...
 *
 * Building a repo takes several git subprocesses (init, configs, add,
 * commit), and a `beforeEach` that does it pays that per test. So
 * build one pristine repo per process and copy it. A directory copy
 * of a repo this size is a few milliseconds and, more to the point,
 * spawns nothing. The template is memoized for the life of the
 * worker, so a file with many tests pays for one repo rather than
 * many.
 *
