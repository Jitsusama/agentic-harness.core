...
import { cite } from "../../result/cite.js";
import { offerQueryTool, withdrawQueryTool } from "../../result/follow.js";
...
describe("what a citation tells the reader to do next", () => {
	...
	it("does not promise a call that cannot be made", () => {
		...
		// separate consumers of this library, each loadable alone.
		// Naming the reader when it is absent tells the caller the rest
		// of the data is one call away, and the call does not exist.
		withdrawQueryTool();
		const text = cut();

		expect(text).not.toContain("Query it with");
		expect(text).toContain("cannot be followed");
		expect(text).toContain("No tool in this session");
	});
...
type FollowModule = typeof import("../../result/follow.js");
...
async function separateCopy(tag: string): Promise<FollowModule> {
	const spec = `../../result/follow.js?copy=${tag}`;
