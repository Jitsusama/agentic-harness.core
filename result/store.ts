 ...
 * queries them reads it back, and those are different consumers in
 * the same process holding different instances. So a handle
 * resolves against the directory rather than against whichever
 * instance happened to write it. An in-memory index would have
 * meant a handle cited by one family was unreadable by the query
 * tool, which is the only thing a handle is for.
 *
...
export interface ResultStore {
	...
	// the directory is shared by every consumer in the session, so
...
export function createResultStore(deps: {
	...
	// shared with every other consumer in the session, so anything
