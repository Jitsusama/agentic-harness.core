 ...
 *
 * The extension owns its own section schema and defaults. Unlike
 * agentic-harness.pi, this package has no single package-config
 * envelope of its own to key a section into, so `SectionParse` is
 * defined locally instead of imported from a config loader -- the
 * shape is generic enough that duplicating the one-line type is
 * cheaper than a dependency on a file envelope this package does
 * not have.
 */

import { join } from "node:path";

/** An extension's parser for its own config section. */
