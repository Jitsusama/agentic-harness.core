/**
 * Stage-machine verbs: think, draft, build, conclude, retire,
 * reopen. Plus the primary-plan pinning helper and the
 * quest-scoped concludeOrRetire (which delegates to
 * stageTransition for document-scoped calls).
 *
 * pi's own `verbs/stage.ts` threads an `ExtensionContext`
 * through `stageTransition`/`concludeOrRetire`, but never
 * actually reads it -- it's dead parameter plumbing left over
 * from an older signature. Dropped here; nothing in this file
 * needs a host context.
 */

import { existsSync, readFileSync } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { nowYmd } from "../../internal/quest/dates.js";
import { discoverQuests } from "../../internal/quest/discovery.js";
import { parseQuestFrontMatter } from "../../internal/quest/frontmatter.js";
import { isWithin } from "../../internal/quest/git-signals.js";
import { mutateQuestFrontMatter } from "../../internal/quest/mutate.js";
import { reapQuestScratchDir } from "../../internal/quest/scratch.js";
import { isSealedStatus } from "../../internal/quest/status.js";
import {
	type JournalChange,
	recordStructuralOp,
} from "../../internal/quest/structural-journal.js";
import {
	listTreesOnQuest,
	removeTreeFromQuest,
	setPendingPrune,
} from "../../internal/quest/trees.js";
import { resolveTreeProvider } from "../../tree/index.js";
import { count, noun, verb } from "../../ui/count.js";
 ...
 * enforced at write time by the write classifier (each
 * adapter's own), not at the transition, so crossing into
 ...
 * stage machine stays in sync, then drive the document
 ...
 *
 * The id comes from the caller, so the resolved path is contained
 * to the quest's subdirectory: a crafted id carrying path separators
 * or `..` resolves outside it and is refused, never reaching the
 * filesystem. Any prior focus is restored afterward so concluding a
 * sibling document does not silently move the caller's focus.
 */
function concludeDocumentById(
	...
	const result = stageTransition(state, action, params);
	// Restore the focus the caller had before, unless they were already
...
export async function concludeOrRetire(
	...
	// A single document-kind id (PLAN/RSCH/BRIF/RPRT) concludes that
	// document (not undoable, see concludeDocumentById); a quest id
	// or any comma list runs the reversible status sweep.
	...
	if (targetId.length > 0) {
		...
		if (declaredScope === "document") {
			...
			return concludeDocumentById(state, action, params, targetId, subdir);
		}
