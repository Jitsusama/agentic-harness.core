 ...
 * runs, by precedence. An explicit quest-verify field wins because
 * it is the most specific to the work in hand; then an explicit
...
export interface CheckCommandSources {
	/** A verify command declared in the caller's own project convention (e.g. a loaded quest's frontmatter). */
...
/**
 * Resolve the check command, or null when nothing is available
 * and the caller has nothing to run.
 */
