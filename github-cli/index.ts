...
import { findFlag, type SimpleCommand, tokenize } from "../command/index.js";
import {
	extractBodyFilePath,
	hasUnquotedHeredoc,
	stripHeredocBodies,
...
/**
 * Run every github-cli check against a raw bash command. Returns
 * the first violation's block reason, or null if the command is
 * fine. The one entry point an adapter needs; the individual
 * `detect*` functions stay exported for adapters that want finer
 * control.
 */
export function checkGithubCli(command: string): string | null {
	...
	return (
		...
		detectUnsafeHeredoc(stripped, command) ??
		null
	);
}
