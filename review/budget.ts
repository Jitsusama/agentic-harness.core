...
import { CLOCK_FLOOR_MS } from "../clock/index.js";
import type { AskLimit, AskStop, Participant } from "./index.js";
 ...
 * consecutive rounds.
 *
 * Held equal to the pi adapter's subagent runner default
 * (`DEFAULT_RUN_PI_TIMEOUT_MS`), which is host-coupled and stays in
 * the adapter; this module has no subagent runner of its own to defer
 * to, so the number is inlined rather than imported.
 */
export const REVIEWER_BACKSTOP_MS = 45 * 60 * 1000;
 ...
 * pay for it.
 *
 * Held equal to the pi adapter's subagent wrap-up default
 * (`WRAP_UP_TIMEOUT_MS`), inlined for the same reason as the backstop
 * above.
 */
