...
/**
 * Where a round reads the change under review from.
 *
 * The minimal shape every host adapter's own tree/worktree
 * abstraction can produce: a path to read from, and an optional
 * caveat when that path is not provably pinned to the commit under
 * review. Adapters own the richer `RoundTree`/scaffolding types this
 * is drawn from; this module only ever needs these two fields.
 */
